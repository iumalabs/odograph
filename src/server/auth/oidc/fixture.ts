...
// Locally-signed fixture ID tokens + a matching local JWKS (specs/032 research.md) — no real
// Google network call needed. Originally lived in tests/server/fixtures/oidc.ts (still re-exported
// from there for the existing vitest suite); moved here so the dev-only fixture-signin route
// (dev-oidc.ts) can reach it too. Mirrors tests/server/fixtures/webauthn.ts's role for passkey.
