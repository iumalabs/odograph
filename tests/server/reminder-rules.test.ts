...
describe("reminder rule status computation (User Story 2)", () => {
  ...
  it("computes on_track, coming_up, and overdue for a date-based rule", async () => {
    ...
    expect(onTrack.remainingUnit).toBe("days");
    expect(onTrack.remainingValue as number).toBeGreaterThan(90);
    ...
    expect(comingUp.remainingUnit).toBe("days");
    expect(comingUp.remainingValue as number).toBeGreaterThan(0);
    expect(comingUp.remainingValue as number).toBeLessThanOrEqual(10);
    ...
    expect(overdue.remainingUnit).toBe("days");
    expect(overdue.remainingValue as number).toBeLessThan(0);
  });
  ...
  it("computes on_track, coming_up, and overdue for a mileage-based rule using the vehicle's logged odometer history", async () => {
    ...
    expect(onTrack.remainingUnit).toBe("distance");
    expect(onTrack.remainingValue).toBeCloseTo(500, 5);
    ...
    expect(comingUp.remainingUnit).toBe("distance");
    expect(comingUp.remainingValue).toBeCloseTo(50, 5);
  ...
  it("shows not_enough_data for a mileage-only rule on a vehicle with no fuel/service records", async () => {
    ...
    expect(created.remainingValue).toBeNull();
    expect(created.remainingUnit).toBeNull();
  });
  ...
  it("reports the more urgent side when both intervals disagree", async () => {
    ...
    expect(created.remainingUnit).toBe("distance");
    expect(created.remainingValue).toBeCloseTo(-200, 5);
  });
