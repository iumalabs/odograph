...
**Scale/Scope**: Two endpoint pairs (registration options/verify, login options/verify), two new D1
tables, minimal client UI (two buttons, no styling system)
...
| VII. Session/CSP/rate limiting           | Sessions issued via the existing `issueSession` (already HttpOnly/Secure/SameSite=Lax); both new write endpoint pairs pass through the existing rate limiter (`rateLimitByIp`, since neither registration nor login has a session yet at the point they're called) | PASS                     |
