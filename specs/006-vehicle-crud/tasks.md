...
- [ ] T001 Create D1 migration `migrations/0006_vehicles.sql`: `CREATE TABLE vehicles` per
...
- [ ] T003 In `src/server/db/repository.ts`: remove `createProbeResource`,
...
- [ ] T004 [US1] Implement `POST /api/v1/vehicles` and `GET /api/v1/vehicles` in
      ...
      caller's tenant
- [ ] T005 [US1] Wire `vehicles.ts` into `src/server/index.ts` under `/api/v1/vehicles`; remove the
      `_tenant-isolation-probe` import and mount point (research.md)
- [ ] T006 [P] [US1] Create `tests/server/vehicle-crud.test.ts` (creation section): 1. Creating a
...
- [ ] T007 [US2] Implement `GET /api/v1/vehicles/:id` and `PATCH /api/v1/vehicles/:id` in
...
- [ ] T008 [P] [US2] Extend `tests/server/vehicle-crud.test.ts` (read/update section): 1. Listing
...
- [ ] T009 [US3] Implement `DELETE /api/v1/vehicles/:id` in `src/server/routes/v1/vehicles.ts`
...
- [ ] T010 [P] [US3] Extend `tests/server/vehicle-crud.test.ts` (deletion section): 1. A deleted
...
- [ ] T011 [P] Delete `src/server/routes/v1/_tenant-isolation-probe.ts` and
      `tests/server/tenant-isolation.test.ts` (superseded by vehicle-crud.test.ts's isolation
      cases from T008/T010)
- [ ] T012 [P] In `tests/server/passkey-auth.test.ts`, `tests/server/magic-link-auth.test.ts`,
...
- [ ] T013 [P] In `tests/server/rate-limit.test.ts`: change its write-path-under-test from
...
- [ ] T014 [P] Implement `src/client/vehicles.ts`: thin wrapper for the 5 endpoints
      (`listVehicles`, `createVehicle`, `getVehicle`, `updateVehicle`, `deleteVehicle`)
- [ ] T015 Modify `src/client/App.tsx`: in the authenticated view, a vehicle list (name +
...
- [ ] T016 [P] Update `src/server/db/schema.sql` reference copy: add `vehicles`, remove
      `probe_resources`
...
- [ ] T018 Walk through quickstart.md end-to-end against `npm run dev`
